	...
	m_Width(0), m_Height(0), m_BPP(0) //初始化
{
	...
	GLCall(glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP)); // 嵌入模式(实际为环绕模式),WRAP_S是水平环绕，如果我们想要嵌入这意味着我们不希望它扩大区域，另一个选项是平铺
	GLCall(glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP)); // S和T有点像x和y
